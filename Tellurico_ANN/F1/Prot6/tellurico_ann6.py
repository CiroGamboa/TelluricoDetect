...
def export_ann(classifier):
    ...
    with open("tellurico_ann6.json",'w') as json_file:
        json_file.write(json_ann)
        
    # Serialize weights to HDF5
    classifier.model.save_weights("tellurico_ann6.h5")
...
def import_ann(X_test,y_test):
    ...
    json_file = open("tellurico_ann6.json",'r')
    loaded_json_ann = json_file.read()
    json_file.close()
    loaded_classifier = model_from_json(loaded_json_ann)
    
    # Load weights into new model
    loaded_classifier.load_weights("tellurico_ann6.h5")
